...
namespace NS_DataType
{
  ...
  public:
    OccupancyGrid_ () : header(), info(), data()
    {
    };

    OccupancyGrid_ (const ContainerAllocator& allocator) : header(allocator),
        info(allocator), data(allocator)
    {
    };
    ...
    std::vector<unsigned char, typename ContainerAllocator::template rebind<unsigned char>::other> data;
