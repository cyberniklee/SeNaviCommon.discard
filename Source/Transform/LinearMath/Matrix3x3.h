...
#define Matrix3x3Data	Matrix3x3DoubleData 
  ...
  public:
    ...
    TFSIMD_FORCE_INLINE const Vector3&
    getRow (int i) const
    {
    ...
    void
    setFromOpenGLSubMatrix (const tfScalar *m)
    {
    ...
    void
    setValue (const tfScalar& xx, const tfScalar& xy, const tfScalar& xz,
              const tfScalar& yx, const tfScalar& yy, const tfScalar& yz,
              const tfScalar& zx, const tfScalar& zy, const tfScalar& zz)
    {
     ...
     */
    void
    setEulerZYX (const tfScalar& yaw, const tfScalar& pitch,
     ...
     **/
    void
    setEulerYPR (tfScalar eulerZ, tfScalar eulerY, tfScalar eulerX)
    {
    ...
    setIdentity ()
    {
      setValue (tfScalar (1.0), tfScalar (0.0), tfScalar (0.0), tfScalar (0.0),
                tfScalar (1.0), tfScalar (0.0), tfScalar (0.0), tfScalar (0.0),
                tfScalar (1.0));
    }
    ...
    getIdentity ()
    {
      static const Matrix3x3 identityMatrix (tfScalar (1.0), tfScalar (0.0),
                                             tfScalar (0.0), tfScalar (0.0),
                                             tfScalar (1.0), tfScalar (0.0),
                                             tfScalar (0.0), tfScalar (0.0),
                                             tfScalar (1.0));
    ...
    void
    getOpenGLSubMatrix (tfScalar *m) const
    {
    ...
    void
    getRotation (Quaternion& q) const
    {
      ...
      else
      {
        ...
        tfScalar s = tfSqrt (
            m_el[i][i] - m_el[j][j] - m_el[k][k] + tfScalar (1.0));
    ...
    getEulerZYX (tfScalar& yaw, tfScalar& pitch, tfScalar& roll,
     ...
     * @param solution_number Which solution of two possible solutions ( 1 or 2) are possible values*/
    void
    getRPY (tfScalar& roll, tfScalar& pitch, tfScalar& yaw,
            unsigned int solution_number = 1) const
    {
      getEulerYPR (yaw, pitch, roll, solution_number);
    }
    ...
    Matrix3x3
    scaled (const Vector3& s) const
    {
      return Matrix3x3 (m_el[0].x () * s.x (), m_el[0].y () * s.y (),
                        m_el[0].z () * s.z (), m_el[1].x () * s.x (),
                        m_el[1].y () * s.y (), m_el[1].z () * s.z (),
    ...
    Matrix3x3
    transposeTimes (const Matrix3x3& m) const;
    Matrix3x3
    timesTranspose (const Matrix3x3& m) const;

    TFSIMD_FORCE_INLINE tfScalar
    tdotx (const Vector3& v) const
    {
      return m_el[0].x () * v.x () + m_el[1].x () * v.y ()
          + m_el[2].x () * v.z ();
    }
    TFSIMD_FORCE_INLINE tfScalar
    tdoty (const Vector3& v) const
    {
      return m_el[0].y () * v.x () + m_el[1].y () * v.y ()
          + m_el[2].y () * v.z ();
    }
    TFSIMD_FORCE_INLINE tfScalar
    tdotz (const Vector3& v) const
    {
      return m_el[0].z () * v.x () + m_el[1].z () * v.y ()
          + m_el[2].z () * v.z ();
    }
     ...
     */
    void
    diagonalize (Matrix3x3& rot, tfScalar threshold, int maxSteps)
    {
      rot.setIdentity ();
      for (int step = maxSteps; step > 0; step--)
      {
        ...
        tfScalar t = threshold
            * (tfFabs (m_el[0][0]) + tfFabs (m_el[1][1]) + tfFabs (m_el[2][2]));
        if (max <= t)
        {
     ...
     */
    tfScalar
    cofac (int r1, int c1, int r2, int c2) const
    {
      return m_el[r1][c1] * m_el[r2][c2] - m_el[r1][c2] * m_el[r2][c1];
    }
    
    void
    serialize (struct Matrix3x3Data& dataOut) const;

    void
    serializeFloat (struct Matrix3x3FloatData& dataOut) const;

    void
    deSerialize (const struct Matrix3x3Data& dataIn);

    void
    deSerializeFloat (const struct Matrix3x3FloatData& dataIn);

    void
    deSerializeDouble (const struct Matrix3x3DoubleData& dataIn);
  ...
  {
    return Matrix3x3 (tfFabs (m_el[0].x ()), tfFabs (m_el[0].y ()),
                      tfFabs (m_el[0].z ()), tfFabs (m_el[1].x ()),
                      tfFabs (m_el[1].y ()), tfFabs (m_el[1].z ()),
  ...
  {
    return Matrix3x3 (cofac (1, 1, 2, 2), cofac (0, 2, 2, 1),
                      cofac (0, 1, 1, 2), cofac (1, 2, 2, 0),
                      cofac (0, 0, 2, 2), cofac (0, 2, 1, 0),
                      cofac (1, 0, 2, 1), cofac (0, 1, 2, 0),
                      cofac (0, 0, 1, 1));
  }
  ...
  Matrix3x3::inverse () const
  {
    ...
    return Matrix3x3 (co.x () * s, cofac (0, 2, 2, 1) * s,
                      cofac (0, 1, 1, 2) * s, co.y () * s,
                      cofac (0, 0, 2, 2) * s, cofac (0, 2, 1, 0) * s,
                      co.z () * s, cofac (0, 1, 2, 0) * s,
                      cofac (0, 0, 1, 1) * s);
  }
  ...
  {
    return Matrix3x3 (m_el[0].dot (m[0]), m_el[0].dot (m[1]),
                      m_el[0].dot (m[2]), m_el[1].dot (m[0]),
                      m_el[1].dot (m[1]), m_el[1].dot (m[2]),
                      m_el[2].dot (m[0]), m_el[2].dot (m[1]),
                      m_el[2].dot (m[2]));
