...
struct TimeStamp
{
  static Time* pointer(M& m) { (void)m; return 0; }
  static Time const* pointer(const M& m) { (void)m; return 0; }
};

template<typename M>
struct TimeStamp<M, typename boost::enable_if<HasHeader<M> >::type >
{
  static Time* pointer(typename boost::remove_const<M>::type &m) { return &m.header.stamp; }
  static Time const* pointer(const M& m) { return &m.header.stamp; }
  static Time value(const M& m) { return m.header.stamp; }
};
 ...
 */
template<typename M>
inline Time const* timeStamp(const M& m)
{
  return TimeStamp<typename boost::remove_reference<typename boost::remove_const<M>::type>::type>::pointer(m);
}
