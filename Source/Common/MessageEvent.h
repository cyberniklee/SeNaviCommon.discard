...
public:
  ...
  {
    init(message, boost::shared_ptr<StringMap>(), Time::now(), true, DefaultMessageCreator<Message>());
  }

  MessageEvent(const ConstMessagePtr& message, const boost::shared_ptr<StringMap>& connection_header, Time receipt_time)
  {
    init(message, connection_header, receipt_time, true, DefaultMessageCreator<Message>());
  }

  MessageEvent(const ConstMessagePtr& message, Time receipt_time)
  {
    init(message, boost::shared_ptr<StringMap>(), receipt_time, true, DefaultMessageCreator<Message>());
  }

  MessageEvent(const ConstMessagePtr& message, const boost::shared_ptr<StringMap>& connection_header, Time receipt_time, bool nonconst_need_copy, const CreateFunction& create)
  {
    init(message, connection_header, receipt_time, nonconst_need_copy, create);
  }

  void init(const ConstMessagePtr& message, const boost::shared_ptr<StringMap>& connection_header, Time receipt_time, bool nonconst_need_copy, const CreateFunction& create)
  {
    message_ = message;
   ...
   */
  StringMap& getConnectionHeader() const { return *connection_header_; }
  const boost::shared_ptr<StringMap>& getConnectionHeaderPtr() const { return connection_header_; }
