...
namespace NS_NaviCommon
{
  ...
  template<typename M>
    ...
    public:
      ...
      typedef boost::function<MessagePtr
      ()> CreateFunction;
      ...
      void
      init (const ConstMessagePtr& message,
            const boost::shared_ptr<StringMap>& connection_header,
            Time receipt_time, bool nonconst_need_copy,
            const CreateFunction& create)
      {
      ...
      void
      operator= (const MessageEvent<Message>& rhs)
      {
      ...
      void
      operator= (const MessageEvent<ConstMessage>& rhs)
      {
        init (
            boost::const_pointer_cast<Message> (
                boost::static_pointer_cast<ConstMessage> (rhs.getMessage ())),
      ...
      getPublisherName () const
      {
        return
            connection_header_ ?
                (*connection_header_)["callerid"] : s_unknown_publisher_string_;
      }
       ...
       */
      Time
      getReceiptTime () const
      {
        return receipt_time_;
      }
      
      bool
      nonConstWillCopy () const
      {
        return nonconst_need_copy_;
      }
      bool
      getMessageWillCopy () const
      {
        return !boost::is_const<M>::value && nonconst_need_copy_;
      }
      
      bool
      operator< (const MessageEvent<M>& rhs)
      {
