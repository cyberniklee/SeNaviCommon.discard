...
#include <assert.h>
#include "../Callbacks/CallbackQueueInterface.h"
...
namespace NS_NaviCommon
{
  ...
  template<class T, class D, class E>
    ...
    public:
      ...
      int32_t
      add (const D& period, const boost::function<void
      (const E&)>& callback,
           CallbackQueueInterface* callback_queue,
           const VoidConstPtr& tracked_object, bool oneshot);
      void
      remove (int32_t handle);

      bool
      hasPending (int32_t handle);
      void
      setPeriod (int32_t handle, const D& period, bool reset = true);
    ...
    private:
      ...
      bool
      waitingCompare (int32_t lhs, int32_t rhs);
      TimerInfoPtr
      findTimer (int32_t handle);
      void
      schedule (const TimerInfoPtr& info);
      void
      updateNext (const TimerInfoPtr& info, const T& current_time);
      ...
      {
      public:
        TimerQueueCallback (TimerManager<T, D, E>* parent,
                            const TimerInfoPtr& info, T last_expected,
                            T last_real, T current_expected)
            : parent_ (parent), info_ (info), last_expected_ (last_expected),
                last_real_ (last_real), current_expected_ (current_expected),
                called_ (false)
        {
        ...
        call ()
        {
          ...
          
          {
            ...
            Time cb_start = Time::now ();
            info->callback (event);
            Time cb_end = Time::now ();
  ...
  template<class T, class D, class E>
    TimerManager<T, D, E>::TimerManager ()
        : new_timer_ (false), id_counter_ (0), thread_started_ (false),
            quit_ (false)
    {
  ...
  template<class T, class D, class E>
    bool
    TimerManager<T, D, E>::waitingCompare (int32_t lhs, int32_t rhs)
    {
  ...
  template<class T, class D, class E>
    typename TimerManager<T, D, E>::TimerInfoPtr
    TimerManager<T, D, E>::findTimer (int32_t handle)
    {
  ...
  template<class T, class D, class E>
    bool
    TimerManager<T, D, E>::hasPending (int32_t handle)
    {
      boost::mutex::scoped_lock lock (timers_mutex_);
  ...
  template<class T, class D, class E>
    int32_t
    TimerManager<T, D, E>::add (const D& period, const boost::function<void
    (const E&)>& callback,
                                CallbackQueueInterface* callback_queue,
      ...
      
      {
        ...
        if (!thread_started_)
        {
          thread_ = boost::thread (
              boost::bind (&TimerManager::threadFunc, this));
        ...
        
        {
          ...
          waiting_.sort (
              boost::bind (&TimerManager::waitingCompare, this, _1, _2));
        }
  ...
  template<class T, class D, class E>
    void
    TimerManager<T, D, E>::remove (int32_t handle)
    {
      ...
      
      {
        ...
        
        {
          ...
          L_int32::iterator it = std::find (waiting_.begin (), waiting_.end (),
                                            handle);
          if (it != waiting_.end ())
          {
            waiting_.erase (it);
  ...
  template<class T, class D, class E>
    void
    TimerManager<T, D, E>::schedule (const TimerInfoPtr& info)
    {
      boost::mutex::scoped_lock lock (timers_mutex_);
  ...
  template<class T, class D, class E>
    void
    TimerManager<T, D, E>::updateNext (const TimerInfoPtr& info,
                                       const T& current_time)
    {
      ...
      else
      {
        ...
        {
          console.error (
              "Time jumped forward by [%f] for timer of period [%f], resetting timer (current=%f, next_expected=%f)",
              (current_time - info->next_expected).toSec (),
              info->period.toSec (), current_time.toSec (),
  ...
  template<class T, class D, class E>
    void
    TimerManager<T, D, E>::setPeriod (int32_t handle, const D& period,
                                      bool reset)
    {
      boost::mutex::scoped_lock lock (timers_mutex_);
      TimerInfoPtr info = findTimer (handle);
      ...
      while (!quit_)
      {
          ...
          else
          {
            ...
            while (!waiting_.empty () && info && info->next_expected <= current)
            {
              ...
              CallbackInterfacePtr cb (
                  boost::make_shared<TimerQueueCallback> (this, info,
                                                          info->last_expected,
                                                          info->last_real,
                                                          info->next_expected));
        ...
        while (!new_timer_ && T::now () < sleep_end && !quit_)
        {
          ...
          else
          {
            ...
            int32_t remaining_time = std::max (
                (int32_t) ((sleep_end - current).toSec () * 1000.0f), 1);
            timers_cond_.timed_wait (
                lock, boost::posix_time::milliseconds (remaining_time));
          }
        }
