...
namespace NS_NaviCommon
{
  ...
  std::string
  AutoVal::toLower (const std::string& source) const
  {
...
//////////////////////////////////////////////////////////
  ...
  bool
  ConfigFile::read (const char* configFile)
  {
    return read (std::string (configFile));
  }
  
  bool
  ConfigFile::read (const std::string& configFile)
  {
  ...
  void
  ConfigFile::insertValue (const std::string& section, const std::string& entry,
  ...
  const AutoVal&
  ConfigFile::value (const std::string& section, const std::string& entry) const
  {
    
    std::map<std::string, AutoVal>::const_iterator ci = m_content.find (
        toLower (section) + '/' + toLower (entry));
    if (ci == m_content.end ())
  ...
  const AutoVal&
  ConfigFile::value (const std::string& section, const std::string& entry,
                     double def)
  {
    try
    {
    ...
    {
      return m_content.insert (
          std::make_pair (section + '/' + entry, AutoVal (def))).first->second;
    }
  }
  ...
  void
  ConfigFile::dumpValues (std::ostream& out)
  {
  ...
  std::string
  ConfigFile::trim (const std::string& source, char const* delims) const
  {
    std::string result (source);
  ...
  std::string
  ConfigFile::truncate (const std::string& source, const char* atChar) const
  {
  ...
  std::string
  ConfigFile::toLower (const std::string& source) const
  {
    std::string result (source);
    char c = '\0';
    for (unsigned int i = 0; i < result.length (); i++)
