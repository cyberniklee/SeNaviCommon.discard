...
namespace NS_NaviCommon
{
  ...
  void
  CallbackQueue::addCallback (const CallbackInterfacePtr& callback,
  ...
  CallbackQueue::IDInfoPtr
  CallbackQueue::getIDInfo (unsigned long id)
  {
    boost::mutex::scoped_lock lock (id_info_mutex_);
  ...
  void
  CallbackQueue::removeByID (unsigned long removal_id)
  {
    setupTLS ();
  ...
  CallbackQueue::CallOneResult
  CallbackQueue::callOne (Duration timeout)
  {
    setupTLS ();
    TLS* tls = tls_.get ();
      ...
      if (callbacks_.empty ())
      {
        if (!timeout.isZero ())
        {
          condition_.timed_wait (
              lock,
              boost::posix_time::microseconds (timeout.toSec () * 1000000.0f));
        }
  ...
  void
  CallbackQueue::callAvailable (Duration timeout)
  {
    setupTLS ();
    TLS* tls = tls_.get ();
  ...
  CallbackQueue::CallOneResult
  CallbackQueue::callOneCB (TLS* tls)
  {
