...
public:
  ...
  virtual void addCallback(const CallbackInterfacePtr& callback, unsigned long removal_id = 0);
  virtual void removeByID(unsigned long removal_id);
   ...
   */
  CallOneResult callOne(Duration timeout);
   ...
   * this parameter does nothing.
   */
  void callAvailable(Duration timeout);
...
protected:
  ...
  IDInfoPtr getIDInfo(unsigned long id);
